...
function displayPixPayment(data) {
    ...
    pixStatusInterval = setInterval(checkPixPaymentStatus, 20000);
...
function checkPixPaymentStatus() {
    ...
    .then(data => {
        if (data.success && data.status === 'RECEIVED') {
            // Pagamento confirmado - parar polling e redirecionar automaticamente
            clearInterval(pixStatusInterval);
            ...
            $('#pix-success').html(`
                ...
                <div class="mt-3">
                    ...
                    <span class="ml-2">Finalizando agendamento automaticamente...</span>
                </div>
            `);

            // Aguardar 2 segundos antes de redirecionar para dar tempo do usuário ver a confirmação
            setTimeout(() => {
                finalizeBookingAutomatically();
            }, 2000);

        } else if (data.success && data.status === 'PENDING') {
            // Pagamento ainda pendente - atualizar mensagem com contador
            ...
            $('#pix-success').html(`
                ...
                <div class="mt-3">
                    <small class="text-muted">
                        <i class="fas fa-sync-alt fa-spin"></i> 
                        Última verificação: ${currentTime} - Verificando novamente em 5 segundos...
                    </small>
                </div>
            `);
        } else {
            ...
            $('#pix-success').html(`
                ...
                <p>Ocorreu um erro ao verificar o status do pagamento. Tente gerar um novo PIX.</p>
                <div class="mt-3">
    ...
    .catch(error => {
        ...
        // Não parar o polling em caso de erro de rede - tentar novamente
        const currentTime = new Date().toLocaleTimeString('pt-BR');
        $('#pix-success').html(`
            <i class="fas fa-clock fa-3x mb-3"></i>
            <h4>Aguardando Pagamento</h4>
            ...
            <div class="mt-3">
                <small class="text-warning">
                    ...
                    Erro na verificação (${currentTime}) - Tentando novamente em 5 segundos...
                </small>
            </div>
        `);
    });
...
function finalizeBookingAutomatically() {
    ...
    const formData = {
        ...
        status: 'RECEIVED', // PIX confirmado
...
function copyPixCode() {
    ...
    if (navigator.clipboard) {
        navigator.clipboard.writeText(pixCode).then(() => {
            ...
            button.html('<i class="fas fa-check"></i> Copiado!').addClass('btn-success').removeClass('btn-outline-primary');
...
function finalizeBooking() {
    ...
    const formData = {
        ...
        titulo: $('#titulo').val(),
        payment_method: currentPaymentMethod,
        status: currentPaymentMethod === 'presencial' ? $('#presencial-status').val() : 'RECEIVED',
        _token: $('input[name="_token"]').val()
    };
